# YOLO experiments (historical — not shipped)

> These are the **earlier, from-scratch experiments** that predate the trained
> `Empty`/`Full` classifier the app now ships (`detect/yolo/best.onnx`, see the
> [parent README](../README.md)). They train a real YOLO **object detector**
> from the repo's own data. The models here perform poorly and **nothing in
> this folder is wired into the app** — it's kept for reference.
>
> Note: these were built against an older `detect.py` that loaded YOLO `.pt`
> weights via **ultralytics**. The current `detect.py` is **onnxruntime-only**,
> so the `--weights …/best.pt` commands below no longer run as-is — export a
> trained `.pt` to ONNX first (`yolo export model=best.pt format=onnx
> imgsz=640`). The `--classifier` mode still works (it uses the 6-channel CNN).

Instead of asking per cell *is there a pill?* with an empty-box reference (what
the shipped classifier does), these explore drawing a box around each pill.
...
- no hand-drawn boxes are needed — the label of each cell comes straight from
  the hand-reviewed `detect/labels.json`;
...
python3 detect/yolo/experiments/build_gridset.py        # -> dataset/yolo_grid/
python3 detect/yolo/experiments/train.py --data dataset/yolo_grid/pillbox.yaml \
...
grid, but takes each verdict from the accurate classifier (`detect/pipeline.py`)
...
Each box is built like this, for every compartment `detect/labels.json` marks
`pill`:
...
- compartments where the pill is the **same colour as its lid** change almost
  nothing, so those use **hand-drawn boxes** placed by eye at high zoom,
...
`detect/labels.json`, so no occupied compartment is missed and no empty one is
...
python3 detect/yolo/experiments/build_handset.py    # -> dataset/yolo_hand/  (418 boxes, 46 imgs)
python3 detect/yolo/experiments/train.py --data dataset/yolo_hand/pillbox.yaml \
...
![hand-labelled boxes](demo/handset_labels_preview.jpg)
...
1. `make_dataset.py` warps each photo to the canonical top-down 7×3 grid
   (reusing `detect/crop_cells.py`) and, inside every cell that
   `detect/labels.json` marks `pill`, runs the classifier's DoG blob detector
   to find the pill blob(s) and writes YOLO boxes for them. Empty cells get no
   boxes. Training on the **warped grid** (not the raw photo) keeps the loose
   pills scattered across the table — which were never labelled — out of frame,
   so they can't act as unlabelled positives.
2. `train.py` fine-tunes / trains `yolov8n` on the single `pill` class.
3. The old `detect.py` ran the trained model on a new photo (locate → warp →
   YOLO) and reported which of the 21 cells have a detected pill.

```bash
pip install ultralytics
python3 detect/crop_cells.py                        # need the cell crops first
python3 detect/yolo/experiments/make_dataset.py     # -> dataset/yolo/ (~1200 pseudo boxes)
python3 detect/yolo/experiments/train.py            # -> dataset/yolo/runs/pill/weights/best.pt
```

## Results

Trained from scratch (no COCO warm-start), 200 epochs on CPU:
...
For real per-cell status, use the shipped classifier and the app's `/status`
page. This folder is here because a YOLO version is a fun thing to see draw
boxes.
